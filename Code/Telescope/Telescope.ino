#define VERSION 1.0   // 31-7-26 
/* * * * * * * * * * * * * * * * * * * * * * *
 * Code by:  Richard Fred
 * Email:    richn01@msn.com
 * Version:  5.0
 * Date:     23 July 2026
 * Copyright (C) 2020, MIT License
 *
 * Telescope control via BLE Gamepad controler. Uses L298 motor control
 * 1 BLE and Motors working
 * 2 CLI Telescope Goto protocol
 * 3 IMU working LSM303 6 Axis 
 * 4 GPS NEO 6
 * 5 Focus control (Stepper Motor)
 * 
 *
 * HOW TO USE:
...
// BIOS ESP32-C3
#define PIN D0             // 12v Motors with L298N
#define ENABLE1 D1             // 12v Motors with L298N
#define DIRA2 D2           // 12v Motors with L298N
#define DIRA1 D3             // 12v Motors with L298N
...
#define DIRB2 D8            // 12v Motors with L298N
#define DIRB1 D9  // Servo shield output enable pin, | Bootstrap
#define ENABLE2 D10           // 12v Motors with L298N | Bootstrap

// D8/D9 Bootstap pins must be high at Power Up/Boot
// pinMode(LED_BUILTIN, OUTPUT);  // Config Built in Led as an output. NOT avaliable ON ESP32C3
...
void setup() {
  Serial.begin(115200);
  Serial.print(F("--- Telescope Goto... - V"));
 ...
 /*
  ...
  myDFPlayer.begin(FPSerial, true, true);  //SW Serial isACK =true, doReset = true
...
void manageMotors(float dt) {
  ...
  //Serial.println( moveValue, turnValue);
